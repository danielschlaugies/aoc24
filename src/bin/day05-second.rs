...
fn in_order(list: &[Page]) -> bool {
    list.is_sorted_by(|x, y| match x.partial_cmp(y) {
    ...
    match s {
        Ok(s) => {
            ...
            update_string.lines().for_each(|line| {
                let mut numbers: Vec<Page> = line
                    .split(',')
                    .map(|n| {
                        let num = n.parse::<i32>().unwrap();
                        Page {
                            number: num,
                            print_order: page_order.clone(),
                        }
                    })
                    .collect();

                if !in_order(numbers.as_slice()) {
                    numbers.sort_by(|x, y| match x.partial_cmp(y) {
